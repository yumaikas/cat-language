...
namespace Cat
{
    ...
    public class FxnDocList : List<FxnDoc>
    {
        ...
        public void OutputDefs(StreamWriter sw, int nLevel)
        {
            foreach (KeyValuePair<string, FxnDoc> kvp in mFxns)
            {
                FxnDoc f = kvp.Value;
                if ((f.mnLevel <= nLevel) && (f.msImpl != ""))
                    sw.WriteLine(f.ToCatImpl());
            }
        }
    ...
    public class CatHelpMaker
    {
        ...
        public void SaveLibrary(string sOutFile)
        {
            FileStream fOut = File.OpenWrite(sOutFile);
            try
            {
                StreamWriter sw = new StreamWriter(fOut);
                sw.WriteLine("// Auto-Generated Level-1 Cat Library with Tests");
                sw.WriteLine("// Generated by Cat interpreter " + Config.gsVersion);
                sw.WriteLine("// on " + Config.gsDate);
                sw.WriteLine("// http://www.cat-language.com");
                sw.WriteLine("");
                mTable.OutputDefs(sw, 1);
                sw.WriteLine("");
                sw.WriteLine("// Tests");
                sw.WriteLine("");
